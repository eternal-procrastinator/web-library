import { useState } from "react";
import { Link, useNavigate } from "react-router-dom";
import { useMutation } from "@apollo/client";
import { Alert, Button, Card, Col, Flex, Form, Input, Layout, Row, Typography } from "antd";
import { RollbackOutlined } from "@ant-design/icons";
import styled from "styled-components";

import { Footer } from "@/widgets/Footer";
import { StyledButton } from "@/features/StyledButton";
import { SIGNUP } from "@/shared/api/auth";

const StyledFormItem = styled(Form.Item)`
  margin-bottom: 24px;
`

const StyledLabel = styled.label`
  font-size: 18px;
`

export const SignupPage: React.FC = () => {
  const navigate = useNavigate();
  const [form] = Form.useForm();
  const [error, setError] = useState<string | null>(null);

  const [signup, { loading }] = useMutation(SIGNUP, {
    onCompleted: () => navigate("/login"),
    onError: (err) => setError(err.message),
  });

  const handleSubmit = async (values: { email: string; password: string }) => {
    setError(null);
    await signup({ variables: values })
  }

  return (
    <Layout title="Sign up" style={{ backgroundColor: "#e4ecfd", minHeight: "100vh" }}>
      <Row justify="center" style={{ flex: 1, marginBottom: 32 }}>
        <Col xs={20} sm={18} md={14} lg={10} xl={8}>
          <Card style={{ margin: "auto", marginTop: 50 }}>
            <Flex justify="center" style={{ marginBottom: 16 }}>
              <Typography.Title level={2}>Web Library Registration</Typography.Title>
            </Flex>

            <Form form={form} size="large" layout="vertical" onFinish={handleSubmit}>
              <StyledFormItem
                name="email"
                label={<StyledLabel htmlFor="email">Email</StyledLabel>}
                rules={[
                  { required: true, message: 'Please enter your email' },
                  { type: 'email', message: 'Enter a valid email address' },
                ]}
                required
                validateDebounce={1000}
              >
                <Input
                  id="email"
                  type="email"
                  disabled={loading}
                />
              </StyledFormItem>

              <StyledFormItem
                name="password"
                label={<StyledLabel htmlFor="password">Password</StyledLabel>}
                rules={[
                  { required: true, message: 'Please enter your password' },
                  { min: 8, message: 'Password must contain a minimum of 8 characters' },
                  { max: 20, message: 'Password must contain a maximum of 20 characters' },
                  {
                    pattern: /^(?=.*[a-z])(?=.*\d)[A-Za-z\d@$!%*?&]{8,}$/,
                    message: 'The password must contain a minimum of 8 characters, one lowercase letter and one number',
                  },
                ]}
                required
                validateDebounce={1000}
              >
                <Input.Password
                  id="password"
                  disabled={loading}
                />
              </StyledFormItem>

              <Form.Item>
                <Flex justify="center" align="center">
                  <StyledButton type="primary" htmlType="submit" loading={loading}>
                    Sign up
                  </StyledButton>
                </Flex>
              </Form.Item>

              {error && <Alert message="Sign up" type="error" showIcon style={{ marginTop: 16 }} />}
            </Form>

            <Flex justify="center" align="center">
              <Link to="/login">
                <Button type="link" icon={<RollbackOutlined />} style={{ fontSize: 18 }}>
                  Back
                </Button>
              </Link>
            </Flex>
          </Card>
        </Col>
      </Row>

      <Footer />
    </Layout>
  );
}
